...
const tasks = [
  ...
  { id: 't3', name: 'resume.zip', progress: { percentage: 70 }, status: 'paused' as const },
...
describe('UploadProgressTray', () => {
  ...
  it('renders one row per task with name + percentage', () => {
    const wrapper = mount(UploadProgressTray, { props: { tasks } });
    expect(wrapper.findAll('.tray__row')).toHaveLength(3);
    expect(wrapper.text()).toContain('video.mp4');
    expect(wrapper.text()).toContain('resume.zip');
  ...
  it('shows the queue length in the header', () => {
    const wrapper = mount(UploadProgressTray, { props: { tasks } });
    expect(wrapper.find('.tray__head').text()).toMatch(/3/);
  });

  it('emits cancel when clicking cancel on running task', async () => {
    const wrapper = mount(UploadProgressTray, { props: { tasks } });
    await wrapper.findAll('.tray__cancel')[0].trigger('click');
    expect(wrapper.emitted('cancel')?.[0]).toEqual(['t1']);
  });

  it('emits resume when clicking resume on paused task', async () => {
    const wrapper = mount(UploadProgressTray, { props: { tasks } });
    await wrapper.find('.tray__resume').trigger('click');
    expect(wrapper.emitted('resume')?.[0]).toEqual(['t3']);
  });
});
